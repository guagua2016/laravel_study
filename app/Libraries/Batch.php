...
trait Batch
{
    ...
    public function batchKeyName($m)
    {
        if (method_exists($m, 'logkeys')) {
    ...
    public function batchArrayKey($m, $keyName)
    {
        if (is_array($keyName)) {
            $key = [];
            foreach ($keyName as $k) {
    ...
    public function batch($modelClass, $toInsert = [], $toUpdate = [], $toDelete = [], $verbose = [])
    {
                            ...
                            if (count($toInsert) < 10) {
                                foreach ($toInsert as $m) {
                                    Log::info(sprintf("%s::insert", $modelClass), [
                                        'key' => $this->batchArrayKey($m, $keyName)
                                    ]);
                                }
                            } else {
                                Log::info(sprintf("%s::insert and etc", $modelClass), [
                                    'key' => $this->batchArrayKey($toInsert[0], $keyName),
    ...
    public function batchWithInTransation($modelClass, $toInsert = [], $toUpdate = [], $toDelete = [], $verbose = [])
    {
        //
        // prodess delete
        //
