...
trait YingmiShareUpdater1
{
    public function updateShare($uid, $fundId, $poId = 0)
    {
        ...
        $row = YingmiShareDetail::where('ys_uid', $uid)
             ...
             ->first($columns);
        if (!$row) {
            // YingmiShare::where('ys_uid', $uid)->where('ys_fund_id', $fundId)->delete();
            return;
        }
        ...
        $share = YingmiShare::firstOrNew(['ys_uid' => $uid, 'ys_portfolio_id' => $poId, 'ys_fund_id' => $fundId]);
        $share->fill($data);
        $share->save();
    }
    ...
    public function updateShareDetail($uid, $row)
    {
        ...
        if ($share) {
            ...
            if ($share->isDirty()) {
                DirtyDumper::xlogDirty($share, 'share update', ['uid' => $uid, 'ys_share_id' => $shareId]);
                $share->save();
                $modified = true;
            }
    ...
    public function updatePoFundShareDetail($uid, $row)
    {
        ...
        foreach ($funds as $fund) {
            ...
            if ($uid == 1000083515) {
                $this->estimateYield($data['ys_fund_id'], $fund, $data);
            }
        ...
        foreach ($shareDatas as $code => $shareData) {
            ...
            if (isset($shares[$code])) {
                ...
                $share = new YingmiShareDetail($shareData);
                $share->ys_share_frozen = '0.00';
                $share->ys_principal = '0.00';
                $share->ys_yield_uncarried = '0.00';
                $share->ys_yield_accumulated = '0.00';
            }
    ...
    public function estimateYield($fundId, $fund, &$data)
    {
        ...
        $yield = ($fund['nav'] - $value->fv_nav) * $fund['totalShare'];
